...
def test_export_jsonl(tmp_path):
    ...
    export_jsonl(verdicts, str(path))
...
def test_export_excel(tmp_path):
    ...
    export_excel(verdicts, str(path))
...
def test_export_excel_sanitizes_illegal_control_chars(tmp_path):
...
def test_export_excel_sanitizes_xml_unsafe_chars_in_sheet_xml(tmp_path):
...
def test_export_excel_review_sort(tmp_path):
    ...
    export_excel([v1, v2, v3], str(path))
    wb = load_workbook(path)
    ws = wb["总"]
    assert ws.cell(2, 2).value == "b.com"
    assert ws.cell(3, 2).value == "c.com"
    assert ws.cell(4, 2).value == "a.com"
    wb.close()


def test_export_excel_keeps_similar_indicators_adjacent(tmp_path):
...
def test_export_excel_with_diagnostics(tmp_path):
    verdicts = [_make_verdict("evil.com")]
    diag = {"processed_count": 1, "parse_error_count": 0, "skipped_total": 0}
    path = tmp_path / "diag.xlsx"
    export_excel(verdicts, str(path), diagnostics=diag)
    wb = load_workbook(path)
    ws = wb["统计"]
    assert ws.max_row >= 2
    wb.close()
...
def test_export_excel_splits_black_and_false_positive_sheets(tmp_path):
    ...
    export_excel([alive, inactive, pending, gray, false_positive], str(path))
    wb = load_workbook(path)
    black = wb["判黑"]
    gray_sheet = wb["灰"]
    false_sheet = wb["误报"]
...
def test_export_excel_structured_fields_and_route_provider_stats(tmp_path):
...
def test_export_neutralizes_spreadsheet_formula_prefixes(tmp_path):
    ...
    export_excel([verdict], str(xlsx_path))
...
def test_export_jsonl_preserves_raw_values_without_neutralization(tmp_path):
    payload = '=HYPERLINK("http://attacker.invalid","x")'
    verdict = _make_verdict("formula.invalid")
    verdict["family"] = payload

    path = tmp_path / "raw.jsonl"
    export_jsonl([verdict], str(path))
    row = json.loads(path.read_text(encoding="utf-8").splitlines()[0])
    assert row["family"] == payload


def test_excel_review_sheets_include_reason_and_review_columns(tmp_path):
